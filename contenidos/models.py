...
class ComunesMultimedia(object):
    ...
    def _busca_por_coletillas(self, lista_coletillas):
        ...
        for coletilla, extension in lista_coletillas:
            ...
            # Si extension no coincide con ninguna... no hacemos nada
            return
        # mostramos todos los ficheros/extension que coincidan y existan
        for coletilla, extension in lista_coletillas:
...
class Evento(models.Model):
    ...
    imagen = models.ImageField(upload_to="eventos/imagenes", blank=True,
                               null=True)
    ...
    class Meta:
        # CUIDADO CUIDADO: Si aparecen efectos de duplicidad de eventos, hay
        # que quitar esto.  De momento solo lo veo en /admin/contenidos/evento/
        # con lo cual está limitado.  Pero es que si no me aparecen
        # desordenados y es feo.
    ...
    def datos_para_calendario(start, end):
        qs = FechaEvento.objects.filter(fecha_final__gte=start,
...
class FechaEvento(models.Model):
    ...
    fecha_final = models.DateField(blank=True, null=True,
                                   help_text=u"Fecha final del evento "
                                   u"inclusive (de no escribir nada se "
                                   u"pondrá igual a la fecha de inicio)")
    hora_inicio = models.TimeField(blank=True, null=True,
                                   help_text=u"Hora de inicio del evento, "
                                   u"vacío si todo el dia")
    hora_final = models.TimeField(blank=True, null=True,
                                  help_text=u"Hora final del evento (de no "
                                  u"escribir nada se pondrá igual que la "
                                  u"hora de inicio)")
...
class Libro(models.Model):
    ...
    tipo = models.CharField(max_length=40, choices=TIPO.CHOICES,
                            default=TIPO.LIBRO, blank=True, null=True)
    ...
    miniatura = FileBrowseField("portada", max_length=200,
                                blank=True, null=True,
                                extensions=[".jpg", ".jpeg", ".png", ".gif"],
                                directory="bibliografia")
...
class Categoria(models.Model):
    ...
    nombre = models.CharField(max_length=250, blank=True, null=True,
                              help_text=u"El nombre de la categoria, si "
                              u"hay más de uno")
...
class Documento(models.Model):
    ...
    titulo = models.CharField(max_length=250,
                              help_text=u"El título principal del documento")
    slug = models.SlugField(help_text=u"La url que aparecerá en el navegador "
                            u"cuando se visualice el detalle de este documento"
                            u" (autogenerado)")
    autor = models.ForeignKey(Autor, blank=True, null=True)
    descripcion = models.TextField(blank=True, null=True,
                                   help_text=u"Escribir un resumen del "
                                   "documento, si es una entrevista "
                                   "detallar el entrevistado")
    fecha = models.DateField(blank=True, null=True)
    fuente = models.CharField(max_length=250, blank=True, null=True,
                              help_text=u"La fuente del documento, si "
                              u"procede, o el nombre del periódico")
...
class UrlAdjunto(Adjunto):
    url = models.URLField()
    miniatura = FileBrowseField("miniaturas", max_length=200,
                                directory="documentos/miniaturas",
                                help_text="Miniatura del contenido, opcional",
                                blank=True, null=True)
...
class FicheroAdjunto(ComunesMultimedia, Adjunto):
    # NO-FIXME no hay mucha forma de que segun el tipo de contenido cambie la
    # carpeta, y como tendremos media (videos) y documentos, hay que disparar
    # al raiz
    # filename = FileBrowseField("fichero", max_length=200,
    #                            directory="documentos")
    # miniatura = FileBrowseField("miniaturas", max_length=200, blank=True,
    #                            null=True, directory="documentos/miniaturas",
    #                            help_text=u"Miniatura del contenido, si "
    #                            u"procede")
    filename = FileBrowseField("fichero", max_length=200, directory="")
    miniatura = FileBrowseField("miniaturas", max_length=200, directory="",
                                help_text=u"Miniatura del contenido, si "
                                u"procede", blank=True, null=True)
...
class CitaDe(models.Model):
    ...
    descripcion = models.CharField('descripción', max_length=250,
                                   default=u"Discurso", help_text=u"Evento (y "
                                   u"lugar) donde se realizó")
    fecha = models.DateField(help_text=u"Escribir fecha exacta (o fecha de "
                             u"ordenación)")
    fecha_literal = models.CharField(max_length=250, blank=True, null=True,
                                     help_text="Escribir fecha a mostrar (en "
                                     u"blanco si es exacta, escribir algo si "
                                     u"es ambigua o no se conoce la exacta)")
...
class CitaSobre(models.Model):
    ...
    cargo = models.CharField(max_length=250, blank=True, null=True,
                             help_text="Cargo del autor (si procede)")
    fecha = models.DateField(blank=True, null=True, help_text=u"Escribir "
                             u"fecha exacta (o fecha de ordenación)")
    fecha_literal = models.CharField(max_length=250, blank=True, null=True,
                                     help_text="Escribir fecha a mostrar (en "
                                     "blanco si es exacta, escribir algo si "
                                     "es ambigua o no se conoce la exacta)")
...
def asigna_finales_al_guardar(sender, instance, **kwargs):
    ...
    if instance.fecha_final < instance.fecha_inicio:
        instance.fecha_final, instance.fecha_inicio = \
            instance.fecha_inicio, instance.fecha_final
    if instance.hora_final < instance.hora_inicio:
        instance.hora_final, instance.hora_inicio = \
            instance.hora_inicio, instance.hora_final


@receiver(post_save, sender=Evento)
def crea_blog_al_guardar_evento(sender, instance, **kwargs):
    if instance:
        e, created = EntryZinnia.objects.get_or_create(
            slug="evento-%d" % instance.pk)
        if created:
            print "Autocreando entrada", e, e.pk
            e.title = u"Nuevo evento: %s" % instance.titulo
            e.content = u"<p>Se va a producir el siguiente evento durante los " \
                u"días %s.</p><p>%s</p>" % (instance.fecha_simple(),
                                             instance.resumen)
            e.excerpt = "Se ha creado un nuevo evento en la agenda"
            e.categories = CategoryZinnia.objects.filter(
                slug="actividades-externas")
