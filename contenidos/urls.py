...
from contenidos.views import Home, Calendario, EventoDay, Libros, LibroDetalle, Documentos, DocumentoDetalle, BusquedaGeneral
...
urlpatterns = patterns('',
    ...
    url(r'buscador/(?P<tipo>\w+)/(?P<query>.+)$', BusquedaGeneral.as_view(), name="busqueda_general_resultados"),
    url(r'busqueda/(?P<tipo>\w+)/(?P<query>.+)$', Libros.as_view(), name="libro_busqueda"),
    url(r'calendario/(?P<year>\d+)/(?P<month>\d+)$', Calendario.as_view(), name='calendario_js'),
    ...
    url(r'dossieres_de_prensa/(?P<slug>[-\w]+)$', DocumentoDetalle.as_view(tipo=TIPO.DOSSIERES_DE_PRENSA), name="dossier_de_prensa_detalle"),
    url(r'evento/(?P<pk>\d+)$', DetailView.as_view(model=Evento), name='evento_view'),
    url(r'evento/(?P<year>\d+)/(?P<month>\d+)/(?P<day>\d+)$', EventoDay.as_view(), name='evento_day'),
