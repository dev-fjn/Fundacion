...
from contenidos.models import Carrusel, Evento, Libro, Documento, TIPO
    ...
    def get_context_data(self, year, month, day, **kwargs):
        ...
        context.update({
            ...
            'object_list': Evento.objects.filter(
                fechaevento__fecha_inicio__lte=now,
                fechaevento__fecha_final__gte=now),
            })
        return context
...
class LibrosBase(object):
    ...
    def get_queryset(self):
        return super(LibrosBase, self).get_queryset().\
            filter(precio__isnull=self.externos)

    def get_context_data(self, **kwargs):
        context = super(LibrosBase, self).get_context_data(**kwargs)
        context.update(self.extra_context)
        autores = set(Libro.objects.values_list('autor', flat=True))
        isbns = set(Libro.objects.filter(isbn__isnull=False).
    ...
    def dispatch(self, *args, **kwargs):
        # Esto es para la vista del buscador. El resto de urls.py tienen
        # "externos" puesto.
        if self.externos is None:
            self.externos = (self.kwargs.get('tipo') ==
                             self.CONTEXT_EXTERNOS['url'])
...
class Libros(LibrosBase, ListView):
    ...
    def dispatch(self, *args, **kwargs):
        # si nos pasan rollos por GET, nos los quitamos de encima y redirigimos
        # por URL
        query = self.request.GET.get('query', '')
        if query:
            kwargs = dict(tipo=self.extra_context['url'], query=query)
            return redirect(reverse('libro_busqueda', kwargs=kwargs))
        return super(Libros, self).dispatch(*args, **kwargs)

    def get_queryset(self):
        qs = super(Libros, self).get_queryset()
        self.valor = self.kwargs.get("query", "").strip() or \
            self.request.GET.get("query", "")
        if self.valor:
            qs = qs.filter(Q(titulo__icontains=self.valor) |
                           Q(autor__icontains=self.valor) |
                           Q(resumen__icontains=self.valor) |
                           Q(isbn__icontains=self.valor))
        self.count = qs.count()
        return qs
...
class DocumentosBase(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tipo"] = TIPO.DICT[self.tipo] \
            if self.tipo in TIPO.DICT else "(Ninguno)"
        context["base_tipo"] = TIPO.BASES_HORMIGAS[self.tipo] \
            if self.tipo in TIPO.BASES_HORMIGAS else "(Ninguno)"
        return context
...
class Documentos(DocumentosBase, ListView):
    ...
    def get_queryset(self):
        ...
        if self.query:
            qs = qs.filter(Q(titulo__icontains=self.query) |
...
class BusquedaGeneral(ListView):
    ...
    def get_queryset(self):
        self.query = self.kwargs.get("query", "").strip() or \
            self.request.GET.get("query", "")
        ...
        self.qs = {
            ...
            'libros': busqueda_exhaustiva(Libro, 'titulo', 'autor', 'resumen',
                                          'isbn'),
            'documentos': busqueda_exhaustiva(Documento, 'titulo',
                                              'autor__nombre', 'descripcion',
                                              'fuente'),
        }
