    ...
    def as_view(cls, **initkwargs):
        ...
        for key in initkwargs:
            ...
            if not hasattr(cls, key):
                raise TypeError('%s() received an invalid keyword %r. as_view only accepts arguments that are already attributes of the class.'
                                 % (cls.__name__, key))
    ...
    def http_method_not_allowed(
        ...
        **kwargs
        ...
        logger.warning('Method Not Allowed (%s): %s', request.method,
                       request.path, extra={'status_code': 405,
...
class TemplateResponseMixin(object):
    ...
    def get(
        ...
        **kwargs
        ...
        from edit import FormView, FormSetView, ModelFormSetView, \
        ...
        if isinstance(self, DetailView):
            update(self.detail_get(*args, **kwargs))
        if isinstance(self, FormView):
            update(self.form_get(*args, **kwargs))
        if isinstance(self, (FormSetView, ModelFormSetView)):
            update(self.formset_get(*args, **kwargs))
        if isinstance(self, CreateView):
            update(self.create_get(*args, **kwargs))
        if isinstance(self, UpdateView):
            update(self.update_get(*args, **kwargs))
        if isinstance(self, ListView):
            update(self.list_get(*args, **kwargs))
