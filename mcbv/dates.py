...
class YearMixin(object):
    ...
    def get_next_year(self, date):
        ...
        return _get_next_prev(self, date, is_previous=False,
                              period='year')
    ...
    def get_previous_year(self, date):
        ...
        return _get_next_prev(self, date, is_previous=True,
                              period='year')
...
class DateMixin(object):
    ...
    def uses_datetime_field(self):
        ...
        model = (self.get_queryset().model if self.model
                 is None else self.model)
    ...
    def _make_single_date_lookup(self, date):
        ...
        if self.uses_datetime_field:
            ...
            return {'%s__gte' % date_field: since, '%s__lt' \
                    % date_field: until}
        else:
...
class BaseDateListView(MultipleObjectMixin, DateMixin, View):
    ...
    def get(
        ...
        **kwargs
        ...
        context = \
            self.get_list_context_data(object_list=self.object_list,
    ...
    def get_dated_queryset(self, ordering=None, **lookup):
        ...
        if not allow_future:
            now = \
                (timezone.now() if self.uses_datetime_field else timezone_today())
        ...
        if not allow_empty:
            ...
            is_empty = (len(qs) == 0 if paginate_by
                        is None else not qs.exists())
            if is_empty:
                raise Http404(_('No %(verbose_name_plural)s available')
                              % {'verbose_name_plural'
                              : force_text(qs.model._meta.verbose_name_plural)})
...
class BaseYearArchiveView(YearMixin, BaseDateListView):
    ...
    def get_dated_items(self):
        ...
        lookup_kwargs = {'%s__gte' % date_field: since, '%s__lt' \
                         % date_field: until}

        qs = self.get_dated_queryset(ordering='-%s' % date_field,
                **lookup_kwargs)
        ...
        return (date_list, qs, {'year': date, 'next_year'
                : self.get_next_year(date), 'previous_year'
                : self.get_previous_year(date)})
...
class BaseMonthArchiveView(YearMixin, MonthMixin, BaseDateListView):
    ...
    def get_dated_items(self):
        ...
        return (date_list, qs, {'month': date, 'next_month'
                : self.get_next_month(date), 'previous_month'
                : self.get_previous_month(date)})
...
class BaseWeekArchiveView(YearMixin, WeekMixin, BaseDateListView):
    ...
    def get_dated_items(self):
        ...
        return (None, qs, {'week': date, 'next_week'
                : self.get_next_week(date), 'previous_week'
                : self.get_previous_week(date)})
...
class BaseDayArchiveView(YearMixin, MonthMixin, DayMixin,
    BaseDateListView):
...
class BaseDateDetailView(YearMixin, MonthMixin, DayMixin, DateMixin,
    ...
    def get_detail_object(self, queryset=None):
        ...
        return super(BaseDetailView,
                     self).get_detail_object(queryset=qs)


class DateDetailView(SingleObjectTemplateResponseMixin,
    BaseDateDetailView):
...
def _get_next_prev(
    ...
    # naive result
        ...
        qs = \
            generic_view.get_queryset().filter(**lookup).order_by(ordering)
