...
class SingleObjectMixin(ContextMixin):
    ...
    def get_queryset(self, model):
        ...
        if model:
            ...
            raise ImproperlyConfigured('%(cls)s is missing a queryset. Define %(cls)s.detail_model, %(cls)s.detail_queryset, or override %(cls)s.get_detail_queryset().'
    ...
    def get_detail_context_data(self, **kwargs):
        ...
        context = {}
        context_object_name = \
            self.get_detail_context_object_name(self.detail_object)
...
class SingleObjectTemplateResponseMixin(TemplateResponseMixin):
    ...
    def get_template_names(self):
        return self._get_template_names(self.detail_object,
                self.detail_model)
    ...
    def _get_template_names(self, object=None, model=None):
        ...
        * ``<app_label>/<object_name><template_name_suffix>.html``
        ...
        if object and self.template_name_field:
            name = getattr(self.detail_object,
        ...
        if isinstance(object, models.Model):
            names.append('%s/%s%s.html' % (object._meta.app_label,
                         object._meta.object_name.lower(),
                         self.template_name_suffix))
        elif model is not None and issubclass(model, models.Model):
            names.append('%s/%s%s.html' % (model._meta.app_label,
