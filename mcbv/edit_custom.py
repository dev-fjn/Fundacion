...
class SearchFormViewMixin(BaseFormView):
    ...
    def get_form_kwargs(self):
        ...
        if req.method in ('POST', 'PUT'):
            ...
            get = dict((k, v) for (k, v) in req.GET.items() if k
                       not in self.ignore_get_keys)
            if get:
...
class OwnObjMixin(SingleObjectMixin):
    ...
    def permission_error(self):
        ...
        return HttpResponse("You don't have permissions to access this %s."
                             % name)
...
class SearchEditFormset(SearchFormView):
    ...
    def form_valid(self, form):
        ...
        if self.request.method == 'GET':
            formset = \
                self.formset_class(queryset=self.get_queryset(form))
        else:
            form.save()
            messages.success(self.request,
                             '%s(s) were updated successfully'
                             % self.model.__name__.capitalize())
            formset = form
